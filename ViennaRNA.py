# Python wrapper for the Vienna RNA Package by Andreas R. Gruber, Ronny
# Lorenz, Stephan H. Bernhart, Richard Neubck, and Ivo L. Hofacker (NAR,
# 2008).
...
class ViennaRNA(dict):
    ...
    def mfe(self, strands, Temp=37.0, dangles="all", outputPS=False):
        ...
        (strands, bp_x, bp_y) = self.convert_bracket_to_numbered_pairs(
            bracket_string)

        energy = float(
            words[len(words) - 1].replace(")", "").replace("(", "").replace("\n", ""))
    ...
    def energy(self, strands, base_pairing_x, base_pairing_y, Temp=37.0, dangles="all"):
        ...
        bracket_string = self.convert_numbered_pairs_to_bracket(
