...
def run():
    ...
    if len(input_args) == 4:
        ...
        output = "Usage: " + \
            input_args[
                0] + " [Pre-Sequence] [Post-Sequence] [Translation Initiation Rate]" + "\n"
        return (False, output)
    ...
    if TIR <= 0:
        return (False, "invalid TIR")
    if len(pre_seq) <= 0 or not all([s in valid_chars for s in pre_seq[:]]):
        return (False, "invalid pre_seq")
    if len(post_seq) <= 0 or not all([s in valid_chars for s in post_seq[:]]) or post_seq[0:3].upper() not in valid_start_codons:
        return (False, "invalid post_seq")

    total_iterations = 0
    MaxIter = 10000
    (TIR_out, RBS_out, estimator, iterations) = Monte_Carlo_Design(
        pre_seq, post_seq, None, TIR, MaxIter, verbose=False)
    total_iterations += iterations

    # If RBS is not designed to specs within 10000 iterations, then start from
...
if __name__ == "__main__":
    ...
    # Prints to standard output -- captured by globusrun interactive and
    # transferred to the submitting computer.
    start = time.clock()
