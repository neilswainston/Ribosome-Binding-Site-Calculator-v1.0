# Python wrapper for NUPACK 2.0 by Dirks, Bois, Schaeffer, Winfree, and
# Pierce (SIAM Review)
...
class NuPACK(dict):
    ...
    def __init__(self, Sequence_List, material):
        ...
        if not material == 'rna' and not material == 'dna' and not material == "rna1999":
            raise ValueError(
                "The energy model must be specified as either ""dna"", ""rna"", or ""rna1999"" .")
        ...
        long_id = "".join(
            [random.choice(string.letters + string.digits) for x in range(10)])
    ...
    def complexes(self, MaxStrands, Temp=37.0, ordered="", pairs="", mfe="", degenerate="", dangles="some", timeonly="", quiet="", AdditionalComplexes=[]):
        ...
        if int(MaxStrands) <= 0:
            raise ValueError(
                "The maximum number of strands must be greater than zero.")
    ...
    def mfe(self, strands, Temp=37.0, multi=" -multi", pseudo="", degenerate="", dangles="some"):
        ...
        # Call NuPACK C programs
        cmd = "mfe"
        args = " -T " + str(Temp) + multi + pseudo + \
            " -material " + material + degenerate + dangles + " "
    ...
    def _write_input_energy(self, strands, base_pairing_x, base_pairing_y):
        ...
        # base_pairing_x and base_pairing_y is a list of base pairings of the
        # strands s.t. #x < #y are base paired
    ...
    def _write_input_complexes(self, MaxStrands, AdditionalComplexes=[]):
        ...
        if len(AdditionalComplexes) > 0:
            ...
            for (complexes, i) in zip(AdditionalComplexes, range(len(AdditionalComplexes))):
                ...
                # Remove complexes if they have less than MaxStrands strands.
                if len(complexes) <= MaxStrands:
                    AdditionalComplexes.pop(i)
                else:
                    counts.append([])
                    added.append(0)
                    # Count the number of each unique strand in each complex
                    # and save it to counts
            ...
            for i in range(len(counts) - 1):
                if added[i] == 0:
                    list_str = list_str + "C " + \
                        " ".join([str(count) for count in counts[i]]) + "\n"
                    list_str = list_str + \
                        " ".join([str(strand)
                                  for strand in AdditionalComplexes[i]]) + "\n"
                    added[i] = 1
                    for j in range(i + 1, len(counts) - 1):
                        if counts[i] == counts[j] and added[j] == 0:
                            list_str = list_str + \
                                " ".join(
                                    [str(strand) for strand in AdditionalComplexes[j]]) + "\n"
                            added[j] = 1
    ...
    def _read_output_cx(self):
        ...
        # Output: strand composition of unordered complexes in key
        # "unordered_complexes"
    ...
    def _read_output_ocx(self):
        ...
        # Output: number of permutations and strand composition of ordered
        # complexes in key "ordered_complexes"
    ...
    def Convert_to_ct(self, complex_ID, name, filename="temp.ct", program="ordered"):
        ...
        # hacksy way of reading from data produced by 'complex', by 'mfe', or
        # by 'subopt'
        ...
        # Determine concatenated sequence of all strands, their beginnings, and
        # ends
        allseq = ""
        strand_begins = []
        strand_ends = []
        ...
        for i in range(1, seq_len + 1):
            ...
            if i in bp_x or i in bp_y:
                if i in bp_x:
                    nt_bp = bp_y[bp_x.index(i)]
                if i in bp_y:
                    nt_bp = bp_x[bp_y.index(i)]
            else:
                nt_bp = 0
            ...
            if i in bp_x or i in bp_y:  # helix continued condition (c,d)
                if i in bp_x:
                    bp_i = bp_y[bp_x.index(i)]
                if i in bp_y:
                    bp_i = bp_x[bp_y.index(i)]

                if bp_i + 1 in bp_x or bp_i + 1 in bp_y:  # helix condition c
                    if bp_i + 1 in bp_x:
                        hc_3p = bp_y[bp_x.index(bp_i + 1)]
                    if bp_i + 1 in bp_y:
                        hc_3p = bp_x[bp_y.index(bp_i + 1)]

                if bp_i - 1 in bp_x or bp_i - 1 in bp_y:  # helix condition d
                    if bp_i - 1 in bp_x:
                        hc_5p = bp_y[bp_x.index(bp_i - 1)]
                    if bp_i - 1 in bp_y:
                        hc_5p = bp_x[bp_y.index(bp_i - 1)]

            else:  # helix start or end (a,b)

                if i + 1 in bp_x or i + 1 in bp_y:  # Start, condition a
                    if i + 1 in bp_x:
                        bp_3p = bp_y[bp_x.index(i + 1)]
                    if i + 1 in bp_y:
                        bp_3p = bp_x[bp_y.index(i + 1)]
                ...
                if i - 1 in bp_x or i - 1 in bp_y:  # End, condition b
                    if i - 1 in bp_x:
                        bp_5p = bp_y[bp_x.index(i - 1)]

                    if i - 1 in bp_y:
                        bp_5p = bp_x[bp_y.index(i - 1)]

                    if bp_5p - 1 not in bp_x and bp_5p - 1 not in bp_y:
                        hc_5p = i - 1

            line = str(i) + "\t" + allseq[i - 1] + "\t" + str(nb_5p) + "\t" + str(nb_3p) + "\t" + str(
                nt_bp) + "\t" + str(strand_counter) + "\t" + str(hc_5p) + "\t" + str(hc_3p) + "\n"
